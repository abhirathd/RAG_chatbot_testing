...
class ChromaRAGChatbot {
    ...
    async initializeChroma() {
        ...
        console.log(`⏱️ [${new Date().toISOString()}] Connecting to ChromaDB at ${CHROMA_URL}...`);
        ...
        try {
            ...
            this.chroma = new ChromaApi(config);
            ...
            try {
                ...
                // Fallback test - try to list collections
            ...
            const connectTime = Date.now() - initStartTime;
            console.error(`❌ ChromaDB connection failed after ${connectTime}ms:`, error.message);
    ...
    async loadExistingCollection() {
        try {
            ...
            console.log(`✅ Loaded existing collection with ${this.documentCount} documents`);
        } catch (error) {
...
`;
    ...
    async chat(message) {
        try {
            ...
            const context = relevantDocs.map((result, index) => {
...
- ChromaDB: ${this.chroma ? '✅ Connected' : '❌ Not connected'} (${CHROMA_URL})
